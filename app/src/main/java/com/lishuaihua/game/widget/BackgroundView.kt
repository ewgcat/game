package com.lishuaihua.game.widget
...
import com.lishuaihua.game.widget.bar.Bars
import com.lishuaihua.game.widget.bar.DnBar
import com.lishuaihua.game.widget.boat.UpBar
import java.util.*
...
class BackgroundView(context: Context, attrs: AttributeSet?) : FrameLayout(context, attrs) {
    val BAR_WIDTH_FACTOR = 0.22F

    val BAR_HEIGHT_FACTOR = 0.35F

    val BAR_GAP_FACTOR = BAR_HEIGHT_FACTOR

    val BAR_MOVE_DURATION_MILLIS = 4000L

    val BAR_APPEAR_INTERVAL_MILLIS = (BAR_MOVE_DURATION_MILLIS / 2.2).toLong()

    val FIRST_APPEAR_DELAY_MILLIS = 3000L
    ...
    fun Bars.asArray() = arrayOf(up, down)
